...
const Form = ({ onCloseModal }: FormProps) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      reset();
      onCloseModal();
      toast.success('Благодарим Вас! Мы обязательно свяжемся с вами в ближайшее время.', {
  ...
  return (
    <>
      <form className={cx('form')} onSubmit={handleSubmit(onSubmit)}>
        ...
        <Input name="name" label="Имя" register={register} error={errors.name?.message} />
        <PhoneInput
          name="phone"
          label="Телефон"
        ...
        />
        <Button type="submit" variant="darkV2" disabled={isDisabled}>
          Отправить
        </Button>
      </form>
    </>
